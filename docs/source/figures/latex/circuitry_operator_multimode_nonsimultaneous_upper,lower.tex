...
\begin{quantikz}[row sep={\QuantikzSeparationRow cm,between origins}, align equals at=1.5]
    \qw & \gate{\op{A}}\gategroup[2,steps=2,style={thin,rounded corners,fill=none, inner xsep=2pt},background]{$\Unitary$} & \qw & \qw \\
    \qw & \qw & \gate{\op{B}} & \qw
\end{quantikz}
$\quad \iff$
\end{minipage}%
...
\begin{align}
\Unitary &= (\Identity \otimes \op{B}) (\op{A} \otimes \Identity) \nonumber\\
&= \op{A} \otimes \op{B} \nonumber
\end{align}
\end{minipage}
\end{document}
