...
% These fonts must be installed on the local system.
% (They are not LaTeX fonts).
\usepackage[verbose]{newunicodechar}
% \newfontfamily{\greekfont}[Scale=MatchAveragecase]{JuliaMono}
\newfontfamily{\greekfont}[Scale=MatchAveragecase]{NewComputerModern10}
...
% }
\newcommand{\definegreekitalicisedreplacement}[1]{%
    \newunicodechar{#1}{\makebox[\glyphwidth][c]{\textit{\greekfont#1}}}%
}
...
\definegreekitalicisedreplacement{ς} % var sigma
\definegreekitalicisedreplacement{τ} % tau
\definegreekitalicisedreplacement{υ} % upsilon
\definegreekitalicisedreplacement{ϕ} % var phi
...
\begin{document}
\VerbatimInput[fontshape=n, fontsize=\small, formatcom=\setlength{\lineskiplimit}{-\maxdimen}, baselinestretch=0.65, commandchars=\\\{\}]{\Filename} % commandchars=\\\{\}
