...
Non-linearity is a highly non-trivial issue, as its presence fundamentally changes the structure of the theory. This is because the standard proofs of many key theorems and concepts in quantum mechanics, including the no-signalling theorem, the no-cloning theorem, and the inability to distinguish between non-orthogonal states, depend on its linearity. Therefore, given the non-linearity and non-unitarity of both D-CTCs and P-CTCs, it unsurprising that these prescriptions possess some powerful features with very interesting consequences.

When used as computational resources, time machines provide significant increases in computational speed and efficiency. According to D-CTCs, both classical and quantum computers with access to a CTC can solve :math:`\mathsf{PSPACE}`-complete :cite:p:`brun_computers_2003, aaronson_closed_2009` problems while a quantum computer can solve :math:`\mathsf{NP}`-complete :cite:p:`brun_computers_2003, bacon_quantum_2004` problems efficiently. Alternatively, P-CTCs tell us that CTC-assisted quantum computers, while being able to solve decision problems efficiently in :math:`\mathsf{NP} \cap \mathsf{co}\textsf{-}\mathsf{NP}` and probabilistically in :math:`\mathsf{NP}` :cite:p:`brun_perfect_2012`, can only generally solve problems in :math:`\mathsf{PP}` :cite:p:`aaronson_quantum_2005`. This means that P-CTCs are (putatively) computationally inferior to D-CTCs, though both are still vastly superior to the regular :math:`\mathsf{BQP}` power of quantum computers and :math:`\mathsf{BPP}` power of classical computers.
