...
This example builds upon the four-qubit ripple-carry adder in :ref:`Example: Ripple-carry adder <eg:adder_ripple>`. The implementation here simplifies the algorithm by instead applying copies of the circuit successively to a tetrapartite input state, which is a composition of the augend, addend, carry, and zero states. As a result, the sequence of trace operations (necessary to isolate the output sum and carry states) mixes the pure (vector) input composition, thereby destroying linearity of the evolution in the input states. This means that only mixed states for each qubit in the output sum state can be recovered, and so we can only sum single integers, not *superpositions* of integers. However, keeping the Hilbert space to a smaller total dimensionality at any one time makes the computations *much* faster, and so we can work with significantly larger integers.
